...
def tafseer_video():
    AVIDEOSFoldor = FilerFoldor.objects.filter(name__icontains=VIDEOS).first()
    print(AVIDEOSFoldor)
    TAFSEER_ALQURAN_AVIDEOSFoldor = FilerFoldor.objects.filter(name__icontains=TAFSEER_ALQURAN, parent=AVIDEOSFoldor).first()
    print(TAFSEER_ALQURAN_AVIDEOSFoldor)
    ...
    with open(path, encoding="utf-8") as f:
        ...
        for _ in data:
            ...
            TAFSEER_ALQURAN_File = FilerFile.objects.filter(file__icontains=_["FileName"], folder=TAFSEER_ALQURAN_AVIDEOSFoldor).first()
