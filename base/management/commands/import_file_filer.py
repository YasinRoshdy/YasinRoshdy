...
from filer.settings import FILER_IMAGE_MODEL, FILER_IS_PUBLIC_DEFAULT
...
class FileImporter(object):
    ...
    def import_file(self, file_obj, folder):
        ...
        if iext in [".jpg", ".jpeg", ".png", ".gif"]:
            obj, created = Image.objects.get_or_create(
                original_filename=file_obj.name, file=file_obj, folder=folder, is_public=FILER_IS_PUBLIC_DEFAULT
            )
            if created:
        ...
        if self.verbosity >= 2:
            print(
                "file_created #%s / image_created #%s -- file : %s -- created : %s" % (self.file_created, self.image_created, obj, created)
            )
        return obj
    ...
    def walker(self, path=None, base_folder=None):
        ...
        if self.verbosity >= 1:
            print(
                ("folder_created #%s / file_created #%s / " + "image_created #%s")
                % (self.folder_created, self.file_created, self.image_created)
            )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--path", action="store", dest="path", required=True, help="Import files located in the path into django-filer")
